  ...
  switch (info.type) {
    case READ:
      switch (tag_list[tag.address].command) {
        ...
        case "TA":
          let command_string = `${
            tag_list[tag.address].command
          }${SICS_TERMINATOR}`;
          log(
            `onTagsRequest - command_string: ${command_string}`,
            DEBUG_LOGGING
          );
        ...
        default:
          log(
            `ERROR: onTagRequest - Unexpected error. SICS command type unknown: ${
              tag_list[tag.address].command
            }`
          );
...
function onData(info) {
  ...
  if (!info.hasOwnProperty("tags")) {
    log(
      `onData - No Tags in onData - Likley 'S' command timeout edge case. See profile code comments.`,
      DEBUG_LOGGING
    );
    writeToCache(MSGBUFFER, "");
  ...
  if (SICSERRORS.hasOwnProperty(responseID)) {
    log(
      `ERROR: SICS Error for tag address '${
        tag_list[tag.address].command
      }'. Returned '${responseID}': ${SICSERRORS[responseID]}`
    );
  ...
  switch (tag_list[tag.address].command) {
    case "SI":
      ...
      if (responseID !== "S") {
        ...
        log(
          `ERROR: Unexpected Command Response. SICS response ID expected: ${
            tag_list[tag.address].command
          } Device returned: ${responseID}`
        );
        // Reset MSGBUFFER for next response
        writeToCache(MSGBUFFER, "");
        return { action: ACTIONCOMPLETE };
      ...
      switch (tag_list[tag.address].command) {
        case "I1":
          switch (responseStatus) {
            case RESPONSESTATUS.ACK:
              info.tags.forEach((tagItem) => {
                if (tagItem.address == "I1") {
                  ...
                  tagItem.value = responseValue[
                    tag_list[tagItem.address].offset
                  ].replaceAll('"', "");
                }
            ...
            case RESPONSESTATUS.NACK:
              log(
                `ERROR: Unable to perform SICS Level and Versions read command.`
              );
              info.tags.forEach((tagItem) => {
        ...
        case "I3":
          switch (responseStatus) {
            ...
            case RESPONSESTATUS.NACK:
              log(
                `ERROR: Unable to perform SW Version and Type Definition read command.`
              );
              info.tags.forEach((tagItem) => {
                tagItem.quality = TAGQUALITY.BAD;
              });
        ...
        case "I5":
          switch (responseStatus) {
            ...
            case RESPONSESTATUS.NACK:
              log(
                `ERROR: Unable to perform SW Identification Number read command.`
              );
              tag.quality = TAGQUALITY.BAD;
              break;
          }
        ...
        case "SI":
          switch (responseStatus) {
            ...
            case RESPONSESTATUS.NACK:
              log(
                `ERROR: Unable to perform Stable Weight Immediate read command.`
              );
              info.tags.forEach((tagItem) => {
                tagItem.quality = TAGQUALITY.BAD;
              });
            ...
            case RESPONSESTATUS.OVER:
              log(
                `ERROR: Overload range of Stable Weight Immediate during read.`
              );
              info.tags.forEach((tagItem) => {
                tagItem.quality = TAGQUALITY.BAD;
              });
            ...
            case RESPONSESTATUS.UNDER:
              log(
                `ERROR: Underload range of Stable Weight Immediate during read.`
              );
              info.tags.forEach((tagItem) => {
                tagItem.quality = TAGQUALITY.BAD;
              });
      ...
      switch (tag_list[tag.address].command) {
        ...
        case "Z":
          switch (responseStatus) {
            ...
            case RESPONSESTATUS.OVER:
              log(
                `ERROR: Upper limit of Zero setting range exceeded during command.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
            case RESPONSESTATUS.UNDER:
              log(
                `ERROR: Lower limit of Zero setting range exceeded during command.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
          }
        ...
        case "ZI":
          switch (responseStatus) {
            ...
            case RESPONSESTATUS.OVER:
              log(
                `ERROR: Upper limit of Zero Immediately setting range exceeded during command.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
            case RESPONSESTATUS.UNDER:
              log(
                `ERROR: Lower limit of Zero Immediately setting range exceeded during command.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
          }
        ...
        case "T":
          switch (responseStatus) {
            ...
            case RESPONSESTATUS.OVER:
              log(
                `ERROR: Upper limit of Tare setting range exceeded during command.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
            case RESPONSESTATUS.UNDER:
              log(
                `ERROR: Lower limit of Tare setting range exceeded during command.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
          }
        ...
        case "TI":
          switch (responseStatus) {
            ...
            case RESPONSESTATUS.OVER:
              log(
                `ERROR: Upper limit of Tare Immediately setting range exceeded during command.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
            case RESPONSESTATUS.UNDER:
              log(
                `ERROR: Lower limit of Tare Immediately setting range exceeded during command.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
            case RESPONSESTATUS.L:
              log(
                `ERROR: Unable to perform Tare Immediately setting command. Device responds that it is CERTIFIED.`
              );
              returnAction = { action: ACTIONCOMPLETE };
              break;
          }
