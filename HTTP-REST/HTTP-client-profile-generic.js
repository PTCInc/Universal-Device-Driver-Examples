...
function onValidateTag(info) {
  ...
  if (info.tag.address === LOGGING_LEVEL_TAG.address) {
    ...
    log(
      'onValidateTag - address "' + info.tag.address + '" is valid.',
      DEBUG_LOGGING
    );
    return info.tag;
  }
...
function onTagsRequest(info) {
  ...
  switch (info.type) {
    case READ:
      ...
      switch (info.tags[0].bulkId) {
        case 1:
          ...
          let json = {
            placeholder:
              "This is just a place holder for whatever JSON payload the user needs to implement",
          };
          ...
          if (!request) {
            log(
              `ERROR: onTagRequest - http_request build failed for "${info.tags[0].address}"`
            );
    ...
    case WRITE:
      ...
      log(
        `ERROR: onTagRequest - Write command for address "${info.tags[0].address}" is not supported`
      );
      return { action: ACTIONFAILURE };
    default:
      log(
        `ERROR: onTagRequest - Unexpected error. Command type unknown: ${info.type}`
      );
      return { action: ACTIONFAILURE };
  }
}
...
function onData(info) {
  ...
  if (info.tags == undefined) {
    log(
      `onData - info.tags does not exist. Info: ${JSON.stringify(info)}`,
      DEBUG_LOGGING
    );

    log(
      `ERROR - Unexpected data received without a tag request. Possibly an extra response from a retry event.`
    );
  ...
  try {
    ...
    log(
      `ERROR - Processing message for HTTP response with processHTTPmsg() failure. Msg: ${err}`
    );
  ...
  if (http_response.getResponseCode() !== 200) {
    ...
    log(
      `ERROR: Failed HTTP response. Received HTTP Response Code ${http_response.getResponseCode()}; Reason: ${
        http_response.headers["reason"]
  ...
  tags.forEach(function (tag) {
    ...
    log(
      `onData - Value in payload at address "${tag.address}": ${JSON.stringify(
        value
      )}`,
      VERBOSE_LOGGING
    );
...
class HttpRequest {
  ...
  buildRequest(payload = null) {
    ...
    if ((this.method === null) | (this.host === null) | (this.port === null)) {
      log(
        `HttpRequest.buildRequest - parameters have not been set correctly. method: ${this.method} | host: ${this.host} | port: ${this.port}`,
        DEBUG_LOGGING
      );
      return false;
    }
    ...
    let request = `${this.method} ${this.path} HTTP/1.1${
      this.#HTTP_HEADER_TERMINATOR
    }`;
    ...
    if (header_copy.hasOwnProperty(this.#HEADERS.CONTENTTYPE)) {
      request += `${this.#HEADERS.CONTENTTYPE}: ${
        header_copy[this.#HEADERS.CONTENTTYPE]
      }${this.#HTTP_HEADER_TERMINATOR}`;
      delete header_copy[this.#HEADERS.CONTENTTYPE];
    } else {
      request += `${this.#HEADERS.CONTENTTYPE}: application/json${
        this.#HTTP_HEADER_TERMINATOR
      }`;
    }
    // Add Host header
    request += `${this.#HEADERS.HOST}: ${this.host}:${this.port}${
      this.#HTTP_HEADER_TERMINATOR
    }`;

    // Unless specified Connection will default to keep-alive
    if (header_copy.hasOwnProperty(this.#HEADERS.CONNECTION)) {
      request += `${this.#HEADERS.CONNECTION}: ${
        header_copy[this.#HEADERS.CONNECTION]
      }${this.#HTTP_HEADER_TERMINATOR}`;
      delete header_copy[this.#HEADERS.CONNECTION];
    } else {
      request += `${this.#HEADERS.CONNECTION}: keep-alive${
        this.#HTTP_HEADER_TERMINATOR
      }`;
    }
    ...
    request += `${this.#HEADERS.CONTENTLENGTH}: ${length}${
      this.#HTTP_HEADER_TERMINATOR
    }`;
  ...
  #buildAuthString(username, password) {
    ...
    let authstr = `${this.#HEADERS.AUTHORIZATION}: Basic ${base64}${
      this.#HTTP_HEADER_TERMINATOR
    }`;
    return authstr;
  }
...
class HttpResponse {
  ...
  processHTTPmsg(stringResponse) {
    ...
    if (Object.keys(this.headers).length === 0) {
      ...
      if (
        this.unprocessed.search(
          this.#HTTP_HEADER_TERMINATOR + this.#HTTP_HEADER_TERMINATOR
        ) === -1
      ) {
        return { action: ACTIONRECEIVE };
      }
      this.headers = this.#parseHTTPHeader(
        this.unprocessed.substring(
          0,
          this.unprocessed.indexOf(
            this.#HTTP_HEADER_TERMINATOR + this.#HTTP_HEADER_TERMINATOR
          )
        )
      );
      this.unprocessed = this.unprocessed.slice(
        this.unprocessed.indexOf(
          this.#HTTP_HEADER_TERMINATOR + this.#HTTP_HEADER_TERMINATOR
        ) + (this.#HTTP_HEADER_TERMINATOR + this.#HTTP_HEADER_TERMINATOR).length
      );
      log(
        `processHTTPmsg - HTTP Header Received: ${JSON.stringify(
          this.headers
        )}`,
        VERBOSE_LOGGING
      );
    ...
    if ("Transfer-Encoding".toLowerCase() in this.headers) {
      switch (this.headers["Transfer-Encoding".toLowerCase()]) {
        case "chunked":
          log(
            `processHTTPmsg -Unprocessed Length: ${this.unprocessed.length}`,
            DEBUG_LOGGING
          );
          let result = this.#parseChunkedMsg(this.unprocessed);
          this.msg = this.msg.concat(...result.msg);
          log(
            `processHTTPmsg -Processed Length: ${this.msg.length}`,
            DEBUG_LOGGING
          );
          if (!result.complete) {
            this.unprocessed = result.leftover;
            log(
              `processHTTPmsg - Chunk Msg Not Complete. Unprocessed (post parse) Length: ${this.unprocessed.length}`,
              DEBUG_LOGGING
            );
            return { action: ACTIONRECEIVE };
          }
        ...
        default:
          log(
            `ERROR: onData - Not supported Transfer-Encoding type: ${
              this.headers["Transfer-Encoding".toLowerCase()]
            }`
          );
          return { action: ACTIONFAILURE };
      }
    ...
    // Confirm if full message payload has been received for non-chunk encoded HTTP payloads
      ...
      log(
        `processHTTPmsg - Content-Length Value: ${
          this.headers["Content-Length".toLowerCase()]
        }`,
        DEBUG_LOGGING
      );
      log(
        `processHTTPmsg - Current Msg Length: ${this.msg.length}`,
        DEBUG_LOGGING
      );
      log(
        `processHTTPmsg - New Data Length: ${this.unprocessed.length}`,
        DEBUG_LOGGING
      );
      if (
        this.headers["Content-Length".toLowerCase()] >
        this.msg.length + this.unprocessed.length
      ) {
        ...
        // FAILURE
        log(`ERROR: onData - Received (${
          this.msg.length + this.unprocessed.length
        } bytes) more then Content-Length 
    ...
    // Unsupported format - Unknown message payload type/length to parse
      ...
      log(
        `ERROR: onData - Unknown Transfer-Encoding/Content-Length in HTTP header: ${JSON.stringify(
          this.headers
        )}`
      );
      this.reset();
  ...
  #parseChunkedMsg(msg) {
    ...
    while (msg.length > 0) {
      ...
      if (chunk_size === 0) {
        msg = msg.slice(
          msg.indexOf(this.#CHUNKED_TERMINATOR + this.#CHUNKED_TERMINATOR) +
            (this.#CHUNKED_TERMINATOR + this.#CHUNKED_TERMINATOR).length
        );
        return { complete: true, msg: result };
      }
      // Need to wait for more data to be provided from driver buffer
      else if (
        chunk_size + chunk_header.length + this.#CHUNKED_TERMINATOR.length >=
        msg.length
      ) {
        return { complete: false, msg: result, leftover: msg };
      } else {
        result = result.concat(
          ...msg.substr(
            msg.indexOf(this.#CHUNKED_TERMINATOR) +
              this.#CHUNKED_TERMINATOR.length,
            chunk_size
          )
        );
