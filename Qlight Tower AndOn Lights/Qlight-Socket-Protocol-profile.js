...
function onValidateTag(info) {
  ...
  log(
    `onValidateTag - address "${JSON.stringify(info)}" is valid.`,
    VERBOSE_LOGGING
  );
  return info.tag;
}
  ...
  switch (info.type) {
    ...
    case WRITE:
      ...
      if (tag.address === "reset") {
        log(
          `onTagsRequest - writeFrame: ${TAGS_LIST[tag.address].msg}`,
          DEBUG_LOGGING
        );
...
function onData(info) {
  ...
  if (!info.hasOwnProperty("tags")) {
    log(
      `ERROR: onData - No Tags in onData - Unexpected Message response.`,
      STD_LOGGING
    );
  ...
  if (inboundData.length !== 10) {
    log(
      `ERROR: onData - MSG not complete. MSGBUFFER: ${inboundData}`,
      STD_LOGGING
    );
    return { action: ACTIONCOMPLETE };
  }
  ...
  switch (info.type) {
    ...
    default:
      log(
        `ERROR: onData - Unexpected error. Command type unknown: ${info.type}`
      );
      return { action: ACTIONFAILURE };
  }
}
